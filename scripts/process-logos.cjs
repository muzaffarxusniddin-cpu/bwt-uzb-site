...
async function toWebp(srcPath, slug, srcBuf) {
  const getSharp = () => srcBuf ? sharp(srcBuf) : sharp(srcPath);
  const meta = await getSharp().metadata();
  ...
  await getSharp().resize(resize).webp({ quality: 90 }).toFile(out);
...
async function main() {
  ...
  if (m) {
    ...
    console.log("PNG buffer:", buf.length, "bytes");
    ...
    console.log(`nestone_png: ${meta2.width}x${meta2.height} hasAlpha=${meta2.hasAlpha} avg_rgb=(${r2},${g2},${b2}) avg_a=${a2}`);
    await toWebp(null,"nestone",buf);
  } else {
    console.log("No embedded PNG — skipping nestone.webp");
  }
  ...
  console.log("mirabad-avenue.svg in place");
